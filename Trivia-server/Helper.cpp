...
BUFFER Helper::getJsonBuffer(SOCKET sc)
{
	int byteSize = getBinaryIntPartFromSocket(sc, 4);
	char* myCharArray = getPartFromSocket(sc, byteSize);

	return BUFFER(myCharArray,myCharArray+ byteSize);
}
...
int Helper::getBinaryIntPartFromSocket(SOCKET sc, int bytesNum)
{

	char* s = getPartFromSocket(sc, bytesNum, 0);
	int a;
	std::memcpy(&a, s, sizeof(int));
	return a;
}
...
void Helper::sendBuffer(SOCKET sc, BUFFER buffer)
{
	
	if (send(sc, (char*)buffer.data(), buffer.size(), 0) == INVALID_SOCKET)
	{
